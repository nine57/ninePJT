...
export const SettlementGroupCreatePage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const group = await settlementGroupService.create(Number(orgId), {
        name: name.trim(),
        description: description.trim(),
      });
      navigate(`/organizations/${orgId}/settlements/${group.id}`);
    } catch (err) {
      console.error('정산 그룹 생성 실패', err);
      setError('정산 그룹을 생성하지 못했습니다.');
    } finally {
